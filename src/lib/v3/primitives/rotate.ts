...
//    stay upright."
//
// Implementation is two snapshots: from and to. The only difference
// between them is the scene's `rotation` field. The tween lerps the
// rotation linearly; the SVG renderer applies it as a single
// `rotate(...)` transform on the outer group plus a counter-rotation
// per bar.
