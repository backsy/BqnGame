...
Read this when you start using the `backlog` MCP server (or `backlog` CLI) on
this repo. The MCP is wired up in `.mcp.json` and the binary comes from the
flake dev shell.

The block below is the upstream guidance shipped by `backlog init`. It is
CLI-flavoured; the equivalent MCP tools (`task_create`, `task_list`,
`task_edit`, `document_create`, …) take the same arguments. Use whichever is
ergonomic.
