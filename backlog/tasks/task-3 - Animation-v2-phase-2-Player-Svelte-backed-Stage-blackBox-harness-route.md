...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 src/lib/animations/v2/player.ts exports play(t, stage): Promise<void> with pipelined prepare
- [ ] #2 src/lib/animations/v2/fn-label.ts exports fnExprLabel(fn): string — exhaustive over FnExpr['kind'] with assertNever default
- [ ] #3 src/lib/animations/v2/motions/ exists with black-box.ts exporting blackBox: AnimateStep
- [ ] #4 blackBox plays an actual choreography (input motion, labeled box, output motion) — not an empty Promise
- [ ] #5 animate.ts's blackBoxStub is replaced by the real blackBox import from motions/black-box.ts
- [ ] #6 src/routes/v2-harness/+page.svelte exists, mounts a Stage, builds a hardcoded Trajectory (≥3 steps), has a play button
- [ ] #7 Stage implementation lives in the harness route, not in v2/
- [ ] #8 renderBqnValue handles number and number-array cleanly; other BqnValue variants are placeholder
- [ ] #9 No imports of svelte/components/routes/old-engine into v2/ files
- [ ] #10 Old engine, game route, sandbox route, AnimatedRow, ValueViz, Editor, GlyphPalette untouched
- [ ] #11 pnpm check and pnpm build both pass
<!-- AC:END -->
